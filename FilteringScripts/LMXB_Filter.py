...
def process(pop) -> tuple:
    """Filtering Logic. Returns full population file with calculated formation channels, as well as a .csv with the BH_sol rows"""

    ### BH-MS_Detached filt
    LMXB_FiltCond_Hist = "(((S2_state != 'BH') | (S2_state != 'NS')) & ((S1_state == 'BH') | (S1_state == 'NS'))) & (state == 'RLO2')"
    print('filtering pop for LMXBs')

    solBH_df_Hist = pd.DataFrame()
    try:
        solBH_df_Hist = pop.history.select(where=LMXB_FiltCond_Hist)
    except Exception as e:
        print(f'WARNING!! found no LMXBs : {e}')

    print(f'found {len(solBH_df_Hist)} LMXB systems')

    #### checking for S2 BHs and S1 MS 
    BH_MS_InverseFiltCond_Hist = "(((S2_state == 'BH') | (S2_state == 'NS')) & ((S1_state != 'BH') | (S1_state != 'NS'))) & (state == 'RLO1')"

    print('filtering pop for S2, S1, BH-Sol')
    try:
        solBH_Inverse_df_Hist = pop.history.select(where = BH_MS_InverseFiltCond_Hist)
        if len(solBH_Inverse_df_Hist) != 0: print (f'Warning!! Found {len(solBH_Inverse_df_Hist)} systems which have S1 CO and S2 donor!! ')
    except:
        print('No inverse lmxbs :)')
    ...
    BH_Sol_Mask = (
        ((df['S1_state'] == 'BH')| (df['S1_state'] == 'NS')) & ((df['S2_state'] != 'BH')| (df['S2_state'] != 'NS'))
        &  (df['state'] == 'RLO2')
        &  (df['time'] < 10e9)
    )
...
def main():
    ...
    filtPop_df, PrevRow, AftRow = process(pop)
